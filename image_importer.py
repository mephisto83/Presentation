...
bl_info = {
    "name": "Import Images/Videos as Planes",
    "author": "Florian Meyer (tstscr), mont29, matali, aporter",
    "version": (2, 0, 2),
    "blender": (2, 76, 0),
    "location": "File > Import > Images as Planes or Add > Mesh > Images as Planes",
    "description": "Imports images/videos and creates planes with the appropriate aspect ratio. "
...
class IMPORT_OT_image_importer(Operator, AddObjectHelper):
    """Create mesh plane(s) from image files with the appropiate aspect ratio"""
    bl_idname = "object.image_video_importer"
    bl_label = "Import Images and Videos(soon) as Planes"
    ...
    def generate_paths(self):
        return (fn.name for fn in self.files if is_image_fn(fn.name, self.extension)), self.directory, self.relative
...
def import_images_button(self, context):
    self.layout.operator(IMPORT_OT_image_importer.bl_idname,
                         text="Images/Videos as Planes", icon='TEXTURE')


def register():
    bpy.utils.register_module(__name__)
    bpy.types.VIEW3D_MT_object.append(import_images_button)


def unregister():
    bpy.utils.unregister_module(__name__)
    bpy.types.VIEW3D_MT_object.remove(import_images_button)
